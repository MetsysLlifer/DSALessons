...
void drawPanel(VersionOne* List) {
    ...
    if (List->count >= 0) {
        ...
        totalHeight = (List->count + 1) * (buttonHeight + itemSpacing) + 20; 
    }
    ...
    if (List->count >= 0) {
        for (int i = 0; i <= List->count; i++) {
            ...
            float yPos = panelBounds.y + scrollOffset.y + buttonPad[1] + (i * (buttonHeight + itemSpacing));
            ...
            if ((yPos + buttonHeight) > panelBounds.y && yPos < (panelBounds.y + panelBounds.height)) {
                ...
                if (GuiButton(itemRect, "")) {
                    ...
                    idx = i;
                    displayElem = &List->Elements[i];
                    toDisplayElem = true;
                }
                // Draw element color indicator
                DrawCircle(xPos + buttonPad[0], yPos + buttonPad[0], buttonHeight / 3, List->Elements[i].color);
                // Draw item label
                DrawText(TextFormat("Element [%d]", i), xPos + (buttonPad[0] * 2), yPos + (buttonPad[0]/ 2), 11 , DARKGRAY);
            }
        }
    }
