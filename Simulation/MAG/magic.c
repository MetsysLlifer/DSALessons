...
// Create a Base Element with default Stats
...
// Recalculate Behavior based on Components
...
// 1. SPATIAL FUSION (The New Mechanic + The Old Math)
void PerformSpatialFusion(Entity* entities, int count, int coreIndex, ParticleSystem* ps) {
    ...
    // Accumulators for Thermodynamics
    ...
    for(int i = 1; i < count; i++) {
        ...
        if (dist < FUSION_RADIUS) {
            // MERGE STRUCTURE
            if (s->auxCount < MAX_AUX) {
                s->aux[s->auxCount++] = entities[i].spellData.core;
            }
            
            // MERGE THERMODYNAMICS
    ...
    if(totalInt > 0) {
        ...
        s->intensity = totalInt; // Intensity grows with fusion
    }
    
    RecalculateStats(s);

    // Transform to Projectile
...
Spell FuseSpellData(Spell A, Spell B) {
    ...
    C.intensity = totalIntensity; // Sum intensity
