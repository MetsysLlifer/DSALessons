...
void UpdateEntityPhysics(Entity* e, Vector2 inputDirection, Rectangle* walls, int wallCount) {
    ...
    for (int i = 0; i < wallCount; i++) {
        if (CheckCollisionRecs(hitBox, walls[i])) { e->position.x -= e->velocity.x * dt; e->velocity.x = 0; hitBox.x = e->position.x - hitboxSize / 2; }
    }
    e->position.y += e->velocity.y * dt;
    hitBox.y = e->position.y - hitboxSize / 2;
    for (int i = 0; i < wallCount; i++) {
        if (CheckCollisionRecs(hitBox, walls[i])) { e->position.y -= e->velocity.y * dt; e->velocity.y = 0; }
    }
}
